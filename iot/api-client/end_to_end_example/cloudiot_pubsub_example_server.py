...
class Server:
    ...
    def __init__(self, service_account_json):
        ...
        ).with_scopes(API_SCOPES)
        if not credentials:
            sys.exit(
                "Could not load service account credential "
    ...
    def run(self, project_id, pubsub_subscription):
        ...
        def callback(message):
            ...
            try:
                ...
                print(
                    "Loading Payload ({}) threw an Exception: {}.".format(
                        message.data, e
                    )
                )
                message.ack()
            ...
            self._update_device_config(
                device_project_id, device_region, device_registry_id, device_id, data
            )
