...
def update_regional_secret_expiration(
    ...
    timestamp.FromDatetime(new_expire)

    # Create a field mask to update only the expire_time field
    update_mask = field_mask_pb2.FieldMask(paths=["expire_time"])

    # Update the secret with the new expiration time
    secret = client.update_secret(
        request={
            "secret": {"name": secret_name, "expire_time": timestamp},
            "update_mask": update_mask,
        }
    )

    print(f"Updated secret {secret.name} expiration time to {new_expire}")
