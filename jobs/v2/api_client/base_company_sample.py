...
def create_company(client_service, company_to_be_created):
    try:
        company_created = (
            client_service.companies().create(body=company_to_be_created).execute()
        )
...
def get_company(client_service, company_name):
    try:
        company_existed = client_service.companies().get(name=company_name).execute()
...
def update_company(client_service, company_name, company_to_be_updated):
    try:
        company_updated = (
            client_service.companies()
            .patch(name=company_name, body=company_to_be_updated)
            .execute()
        )
    ...
    try:
        company_updated = (
            ...
            .patch(
                ...
                updateCompanyFields=field_mask,
            )
            .execute()
        )
        print("Company updated: %s" % company_updated)
