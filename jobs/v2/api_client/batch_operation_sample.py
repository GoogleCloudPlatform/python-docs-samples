...
def batch_job_create(client_service, company_name):
    ...
    batch.add(client_service.jobs().create(body=request1), callback=job_create_callback)

    job_to_be_created2 = base_job_sample.generate_job_with_required_fields(company_name)
    request2 = {"job": job_to_be_created2}
    batch.add(client_service.jobs().create(body=request2), callback=job_create_callback)
    batch.execute()
