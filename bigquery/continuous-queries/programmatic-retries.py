...
def retry_continuous_query(event, context):
    ...
    try:
        ...
        timestamp_match = re.search(r"\s*TIMESTAMP\(('.*?')\)(\s*\+ INTERVAL 1 MICROSECOND)?", sql_query)
        ...
        if timestamp_match:
            ...
            sql_query = sql_query.replace("CURRENT_TIMESTAMP() - INTERVAL 10 MINUTE", new_timestamp)
        ...
        data = {
            "configuration": {
                "query": {
                    ...
                    "useLegacySql": False,
                    "continuous": True,
                    "connectionProperties": [{"key": "service_account",
