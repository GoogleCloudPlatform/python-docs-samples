...
def create_dicom_store(
    service_account_json, project_id, cloud_region, dataset_id, dicom_store_id
):
    ...
    request = (
        ...
        .datasets()
        .dicomStores()
        .create(parent=dicom_store_parent, body=body, dicomStoreId=dicom_store_id)
    )
...
def list_dicom_stores(service_account_json, project_id, cloud_region, dataset_id):
    ...
    for dicom_store in dicom_stores:
        print(
            ...
            "Notification config: {}".format(
                dicom_store.get("name"), dicom_store.get("notificationConfig")
            )
        )
