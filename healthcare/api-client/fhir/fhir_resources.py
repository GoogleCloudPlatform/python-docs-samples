...
def create_patient(service_account_json, base_url, project_id, cloud_region,
...
def create_encounter(service_account_json, base_url, project_id, cloud_region,
    ...
    body = {
        ...
        'reason': [{
            'text': 'The patient had an abnormal heart rate. She was concerned'
                    ' about this.'
        }],
        'subject': {
...
def create_observation(
    ...
    body = {
        ...
        'context': {
            'reference': 'Encounter/{}'.format(encounter_id)
        }
    }
...
def delete_resource(service_account_json, base_url, project_id, cloud_region,
                    dataset_id, fhir_store_id, resource_type, resource_id):
    """
...
def get_resource(service_account_json, base_url, project_id, cloud_region,
                 dataset_id, fhir_store_id, resource_type, resource_id):
...
def list_resource_history(service_account_json, base_url, project_id,
    ...
    print('History for {} resource:'.format(
...
def export_resources(service_account_json, base_url, project_id, cloud_region,
                     dataset_id, fhir_store_id, gcs_destination):
    """Exports resources in a FHIR store."""
    url = '{}/projects/{}/locations/{}'.format(
        base_url, project_id, cloud_region)

    resource_path = '{}/datasets/{}/fhirStores/{}'.format(url, dataset_id,
                                                          fhir_store_id)
...
def import_resources(service_account_json, base_url, project_id, cloud_region,
                     dataset_id, fhir_store_id, gcs_source):
    """Exports resources in a FHIR store."""
    url = '{}/projects/{}/locations/{}'.format(
        base_url, project_id, cloud_region)
...
def delete_resource_purge(service_account_json, base_url, project_id,
                          cloud_region, dataset_id, fhir_store_id,
                          resource_type, resource_id):
    ...
    if response.status_code < 400:
        print('Deleted versions of {} resource '
...
def update_resource(service_account_json, base_url, project_id, cloud_region,
                    dataset_id, fhir_store_id, resource_type, resource_id):
...
def conditional_update_resource(service_account_json, base_url, project_id,
                                cloud_region, dataset_id, fhir_store_id,
    ...
    response = session.put(
        resource_path, headers=headers, params=params, json=body)

    response.raise_for_status()
    resource = response.json()

    print('Conditionally updated Observations with the identifier '
...
def conditional_delete_resource(service_account_json, base_url, project_id,
                                cloud_region, dataset_id, fhir_store_id):
...
def patch_resource(service_account_json, base_url, project_id, cloud_region,
                   dataset_id, fhir_store_id, resource_type, resource_id):
...
def conditional_patch_resource(service_account_json, base_url, project_id,
                               cloud_region, dataset_id, fhir_store_id):
    """
    If a resource is found based on the search criteria specified in
    ...
    response = session.patch(
        resource_path, headers=headers, params=params, data=body)
    response.raise_for_status()
...
def search_resources_get(service_account_json, base_url, project_id,
                         cloud_region, dataset_id, fhir_store_id,
                         resource_type):
    """
    ...
    print('Using GET request, found a total of {} {} resources:'.format(
...
def search_resources_post(service_account_json, base_url, project_id,
                          cloud_region, dataset_id, fhir_store_id):
    """
    Searches resources in the given FHIR store using the
    ...
    resource_path = '{}/Patient/_search?family:exact=Smith'.format(
        fhir_store_path)
    ...
    resources = response.json()
    print('Using POST request, found a total of {} Patient resources:'.format(
        resources['total']))
...
def get_patient_everything(service_account_json, base_url, project_id,
                           cloud_region, dataset_id, fhir_store_id,
                           resource_id):
...
def get_metadata(service_account_json, base_url, project_id, cloud_region,
                 dataset_id, fhir_store_id):
...
def execute_bundle(service_account_json, base_url, project_id, cloud_region,
                   dataset_id, fhir_store_id, bundle):
...
def parse_command_line_args():
    ...
    parser.add_argument(
        '--base_url', default=_BASE_URL, help='Healthcare API URL.')
    ...
    parser.add_argument(
        '--cloud_region', default='us-central1', help='GCP region')
    ...
    parser.add_argument(
        '--version_id', default=None, help='Version of a FHIR resource')
    ...
    command.add_parser(
        'list-resource-history', help=list_resource_history.__doc__)
    command.add_parser('execute-bundle', help=execute_bundle.__doc__)
    command.add_parser(
        'get-resource-history',
        help=get_resource_history.__doc__)
    command.add_parser(
        'delete-resource-purge', help=delete_resource_purge.__doc__)
    ...
    command.add_parser(
        ...
        help=search_resources_get.__doc__)
    command.add_parser(
        'get-patient-everything', help=get_patient_everything.__doc__)
