...
def test_image_from_image(autodelete_image_name):
    ...
    assert any(
        feature.type_ == compute_v1.GuestOsFeature.Type.MULTI_IP_SUBNET.name
        for feature in new_image.guest_os_features
    )


def test_image_from_snapshot(test_snapshot, autodelete_image_name):
    img = create_image_from_snapshot(
        PROJECT,
        test_snapshot.name,
        autodelete_image_name,
        guest_os_features=[compute_v1.GuestOsFeature.Type.MULTI_IP_SUBNET.name],
    ...
    assert any(
        feature.type_ == compute_v1.GuestOsFeature.Type.MULTI_IP_SUBNET.name
        for feature in img.guest_os_features
    )


def test_status_change(autodelete_image):
    set_deprecation_status(
        PROJECT, autodelete_image.name, compute_v1.DeprecationStatus.State.DEPRECATED
    )
