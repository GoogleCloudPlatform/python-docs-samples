...
def setup_resources(
    compute, iam, project, test_id, zone, image_family, machine_type, account_email
):

    # Create a temporary service account.
    iam.projects().serviceAccounts().create(
        name="projects/" + project, body={"accountId": test_id}
    ).execute()
    ...
    service_account_key = (
        ...
        .keys()
        .create(
            name="projects/" + project + "/serviceAccounts/" + account_email, body={}
        )
        .execute()
    )
    ...
    firewall_config = {
        ...
        "targetServiceAccounts": [account_email],
    ...
    operation = (
        compute.instances()
        .insert(project=project, zone=zone, body=instance_config)
        .execute()
    )
    ...
    while (
        compute.instances()
        .getIamPolicy(
            project=project, zone=zone, resource=test_id, fields="bindings/role"
        )
        .execute()["bindings"][0]["role"]
...
def cleanup_resources(compute, iam, project, test_id, zone, account_email):
    ...
    try:
        delete = (
            compute.instances()
            .delete(project=project, zone=zone, instance=test_id)
            .execute()
        )

        while (
            compute.zoneOperations()
            .get(project=project, zone=zone, operation=delete["name"])
            .execute()["status"]
            != "DONE"
        ):
            time.sleep(5)
