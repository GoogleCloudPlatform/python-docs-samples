...
"""Example Airflow DAG that performs an export from BQ tables listed in config file to GCS,
copies GCS objects across locations (e.g., from US to EU) then imports from GCS to BQ. The DAG
imports the gcs_to_gcs operator from plugins and dynamically builds the tasks based on the
list of tables.
Lastly, the DAG defines a specific application logger to generate logs.

This DAG relies on three Airflow variables
https://airflow.apache.org/concepts.html#variables
* master_file_path - CSV file listing source and target tables, including Datasets.
* gcs_source_bucket - Google Cloud Storage bucket to use for exporting BigQuery tables in source.
* gcs_dest_bucket - Google Cloud Storage bucket to use for importing BigQuery tables in
                    destination.
...
# 'master_file_path': This variable will contain the location of the master file.
...
def read_master_file(master_file):
  """
  Reads the master CSV file that will help in creating Airflow tasks in the DAG dynamically.
  :param master_file: (String) The file location of the master file,
                                e.g. '/home/airflow/framework/master.csv'
  :return master_record_all: (List) List of Python dictionaries containing the information f
...
with models.DAG('bq_copy_us_to_eu_01',
  ...
  # Loop over each record in the 'all_records' python list to build up Airflow tasks
  for record in all_records:
    ...
    BQ_to_GCS = BigQueryToCloudStorageOperator(
      ...
      destination_cloud_storage_uris=['{}.avro'.format('gs://'+source_bucket+'/'+table_source)],
      export_format='AVRO'
    )
