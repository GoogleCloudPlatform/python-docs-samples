...
def successful_response():
    ...
    with mock.patch('composer2_airflow_rest_api.make_composer2_web_server_request', return_value=response_mock):
        yield
...
def test_trigger_dag_insufficient_permissions(insufficient_permissions_response):
    with pytest.raises(Exception) as e:
        ...
        print(composer2_airflow_rest_api.trigger_dag(COMPOSER2_WEB_SERVER_URL, 'airflow_monitoring', dag_config))
...
def test_trigger_dag_incorrect_environment():
    with pytest.raises(Exception) as e:
        dag_config = DAG_CONFIG
        print(composer2_airflow_rest_api.trigger_dag('https://invalid-environment.composer.googleusercontent.com', 'airflow_monitoring', dag_config))
