...
# This sample creates a workload running on CRfA.
...
@pytest.fixture
def deployed_service(container_image, disable_experimental_warning):
...
def test_end_to_end(crfa_cluster_ingress_address):
    ...
    data = 'diagram.png?dot=digraph G { A -> {B, C, D} -> {F} }'
    data = data.replace(" ", "%20")
    print(f"http://{ingress_addr}/{data}")
    print(f"{service_name}")

    req = request.Request(
        f"http://{ingress_addr}/{data}",
        headers={
