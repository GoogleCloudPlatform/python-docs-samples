...
def get_dataset(project_id, compute_region, dataset_id):
    ...
    print("Dataset id: {}".format(dataset.name.split("/")[-1]))
    print("Dataset display name: {}".format(dataset.display_name))
    print("Entity dataset metadata:")
    print("\t{}".format(dataset.text_extraction_dataset_metadata))
    print("Dataset example count: {}".format(dataset.example_count))
    print("Dataset create time: {}".format(datetime.fromtimestamp(dataset.create_time.seconds).strftime("%Y-%m-%dT%H:%M:%SZ")))
    print("\n")
...
if __name__ == "__main__":
    ...
    list_datasets_parser.add_argument(
        "filter_", nargs="?", default="text_extraction_dataset_metadata:*"
    )
    ...
    if args.command == "create_dataset":
        create_dataset(
            project_id, compute_region, args.dataset_name
        )
