...
DEBUG = false # set to true to test locally using Cloud SQL proxy listening on a TCP port
...
def mysql_demo(request):
    ...
    if not mysql_conn:
        if DEBUG:
            # try to connect using localling running Cloud SQL proxy (local development only)
            mysql_conn = pymysql.connect(**mysql_config, host='127.0.0.1', port=3306)
        else:
            mysql_conn = pymysql.connect(**mysql_config, unix_socket=f'/cloudsql/{CONNECTION_NAME}')
