...
def list_table_specs(project_id, compute_region, dataset_display_name, filter_=None):
    ...
    for table_spec in response:
        ...
        print(
            "Table spec time column spec id: {}".format(table_spec.time_column_spec_id)
        )
...
def list_column_specs(project_id, compute_region, dataset_display_name, filter_=None):
...
def get_table_spec(project_id, compute_region, dataset_id, table_spec_id):
    ...
    print("Table spec name: {}".format(table_spec.name))
    print("Table spec id: {}".format(table_spec.name.split("/")[-1]))
    print("Table spec time column spec id: {}".format(table_spec.time_column_spec_id))
    print("Table spec row count: {}".format(table_spec.row_count))
    print("Table spec column count: {}".format(table_spec.column_count))
...
def import_data(project_id, compute_region, dataset_display_name, path):
    ...
    if path.startswith("bq"):
        ...
        response = client.import_data(
            dataset_display_name=dataset_display_name, gcs_input_uris=input_uris
        )
...
def export_data(project_id, compute_region, dataset_display_name, gcs_uri):
    ...
    response = client.export_data(
        dataset_display_name=dataset_display_name, gcs_output_uri_prefix=gcs_uri
    )
...
def update_table_spec(
    project_id, compute_region, dataset_display_name, time_column_spec_display_name
):
    """Update table spec."""
...
if __name__ == "__main__":
    ...
    if args.command == "get_table_spec":
        get_table_spec(
            project_id, compute_region, args.dataset_display_name, args.table_spec_id
        )
    ...
    if args.command == "export_data":
        export_data(project_id, compute_region, args.dataset_display_name, args.gcs_uri)
