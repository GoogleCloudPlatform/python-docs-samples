...
def test_faces(capsys):
    file_name = os.path.join(
        os.path.dirname(__file__), "resources/face_no_surprise.jpg"
    )
...
def test_faces_http(capsys):
    uri = (
        "https://storage-download.googleapis.com/{}/vision/"
...
def test_detect_web_with_geo(capsys):
    ...
    out = out.lower()
    assert (
        "zepra" in out or "electra tower" in out or "tel aviv" in out or "jaffa" in out
    )
...
def test_async_detect_document(capsys):
    ...
    detect.async_detect_document(
        gcs_source_uri=uri, gcs_destination_uri=GCS_DESTINATION_URI
    )
    out, _ = capsys.readouterr()
