...
from custom_metric import get_now
from custom_metric import get_start_time
...
def test_custom_metric(client, custom_metric):
    # Use a constant seed so pseudo random number is known ahead of time
    random.seed(1)
    ...
    def write_value():
        # Reseed it to make sure the sample code will pick the same value.
        random.seed(1)
        with mock.patch("googleapiclient.http.HttpRequest.execute"):
    ...
    def eventually_consistent_test():
        with mock.patch("googleapiclient.http.HttpRequest.execute", mock.MagicMock(return_value=TIMESERIES)):
