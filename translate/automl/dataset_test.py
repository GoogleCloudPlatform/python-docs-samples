...
def test_dataset_create_import_delete(capsys):
    ...
    automl_translation_dataset.create_dataset(project_id, compute_region, 
        dataset_name, 'en', 'ja')
    ...
    automl_translation_dataset.import_data(project_id, compute_region, 
        dataset_id, data)
    out, _ = capsys.readouterr()
    assert 'Data imported.' in out

    # delete dataset
    automl_translation_dataset.delete_dataset(project_id, compute_region, 
        dataset_id)
    out, _ = capsys.readouterr()
...
def test_dataset_list_get(capsys):
    ...
    automl_translation_dataset.get_dataset(project_id, compute_region, 
        dataset_id)
    out, _ = capsys.readouterr()
