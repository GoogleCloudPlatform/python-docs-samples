...
def test_screen_pdf_file(
    ...
    template_id, _ = basic_sdp_template
    user_prompt_pdf_file_path = os.path.join(
        os.path.dirname(__file__), "test_user_prompt_pdf_base64.txt"
    )
    with open(user_prompt_pdf_file_path) as f:
        user_prompt_in_base64 = f.read()

    response = screen_pdf_file(
        project_id, location_id, template_id, user_prompt_in_base64
    )
