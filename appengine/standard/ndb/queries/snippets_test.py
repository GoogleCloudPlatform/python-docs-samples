...
def test_query_contact_sub_entities_beware(testbed):
    ...
    for contact in contacts:
        assert "Spear St" in [a.street for a in contact.addresses] or "Amsterdam" in [
            a.city for a in contact.addresses
        ]
...
def test_query_contact_multiple_values_in_single_sub_entity(testbed):
    ...
    assert any(
        a.city == "San Francisco" and a.street == "Spear St"
...
def test_query_properties_named_by_string_using_getattr(testbed):
    ...
    query = snippets.query_properties_named_by_string_using_getattr("title", "from")
    articles = query.fetch()
...
def test_order_query_results_by_property(testbed):
    ...
    expando_query, property_query = snippets.order_query_results_by_property("title")
