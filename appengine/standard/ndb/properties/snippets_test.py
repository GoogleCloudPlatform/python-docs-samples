...
def test_account(testbed):
    account = snippets.Account(username="flan", userid=123, email="flan@example.com")
    account.put()


def test_employee(testbed):
    employee = snippets.Employee(full_name="Hob Gadling", retirement_age=600)
    employee.put()
...
def test_notebook(testbed):
    note1 = snippets.Note(text="Refused to die.", when=1389)
    note2 = snippets.Note(text="Printed some things", when=1489)
    note3 = snippets.Note(text="Learned to use a sword", when=1589)

    notebook = snippets.Notebook(notes=[note1, note2, note3])
    stored_notebook = snippets.SignedStorableNotebook(author="Hob Gadling", nb=notebook)
