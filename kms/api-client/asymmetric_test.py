...
def setup_module(module):
    ...
    s2 = create_key_helper(
        t.rsaSignId,
        enums.CryptoKey.CryptoKeyPurpose.ASYMMETRIC_SIGN,
        enums.CryptoKeyVersion.CryptoKeyVersionAlgorithm.RSA_SIGN_PSS_2048_SHA256,
        t,
    )
    s3 = create_key_helper(
        t.ecSignId,
        enums.CryptoKey.CryptoKeyPurpose.ASYMMETRIC_SIGN,
        enums.CryptoKeyVersion.CryptoKeyVersionAlgorithm.EC_SIGN_P256_SHA256,
        t,
    )
...
class TestKMSAsymmetric:
    ...
    rsaSign = "{}/keyRings/{}/cryptoKeys/{}/cryptoKeyVersions/1".format(
        parent, keyring_id, rsaSignId
    )
    rsaDecrypt = "{}/keyRings/{}/cryptoKeys/{}/cryptoKeyVersions/1".format(
        parent, keyring_id, rsaDecryptId
    )
    ecSign = "{}/keyRings/{}/cryptoKeys/{}/cryptoKeyVersions/1".format(
        parent, keyring_id, ecSignId
    )
    ...
    def test_rsa_encrypt_decrypt(self):
        ciphertext = asymmetric.encrypt_rsa(self.message_bytes, self.rsaDecrypt)
        # signature should be 256 bytes for RSA 2048
        assert len(ciphertext) == 256, "ciphertext should be 256 chars; got {}".format(
            len(ciphertext)
        )
        plaintext_bytes = asymmetric.decrypt_rsa(ciphertext, self.rsaDecrypt)
        assert plaintext_bytes == self.message_bytes
        plaintext = plaintext_bytes.decode("utf-8")
        assert plaintext == self.message

    def test_rsa_sign_verify(self):
        sig = asymmetric.sign_asymmetric(self.message_bytes, self.rsaSign)
        # signature should be 256 bytes for RSA 2048
        assert len(sig) == 256, "sig should be 256 chars; got {}".format(len(sig))
        success = asymmetric.verify_signature_rsa(sig, self.message_bytes, self.rsaSign)
        assert success is True, "RSA verification failed"
        changed_bytes = self.message_bytes + b"."
        success = asymmetric.verify_signature_rsa(sig, changed_bytes, self.rsaSign)
        assert success is False, "verify should fail with modified message"

    def test_ec_sign_verify(self):
        sig = asymmetric.sign_asymmetric(self.message_bytes, self.ecSign)
        assert len(sig) > 50 and len(sig) < 300, "sig outside expected length range"
        success = asymmetric.verify_signature_ec(sig, self.message_bytes, self.ecSign)
        assert success is True, "EC verification failed"
        changed_bytes = self.message_bytes + b"."
        success = asymmetric.verify_signature_ec(sig, changed_bytes, self.ecSign)
        assert success is False, "verify should fail with modified message"
