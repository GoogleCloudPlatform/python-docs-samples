...
class TestKMSSnippets:
    ...
    @pytest.mark.skip(
        reason="There's currently no method to delete keyrings, \
    ...
    @pytest.mark.skip(
        reason="Deleting keys isn't instant, so we should avoid \
    ...
    def test_create_crypto_key(self):
        ...
        snippets.create_crypto_key(
            self.project_id, self.location, self.keyring_id, key_id
        )
        c = kms_v1.KeyManagementServiceClient()
        result = c.get_crypto_key(
            c.crypto_key_path(self.project_id, self.location, self.keyring_id, key_id)
        )
    ...
    def test_key_change_version_state(self):
        ...
        name = client.crypto_key_version_path(
            self.project_id, self.location, self.keyring_id, self.symId, self.version
        )
    ...
    def test_get_ring_policy(self):
        policy = snippets.get_key_ring_policy(
            self.project_id, self.location, self.keyring_id
        )
    ...
    def test_ring_policy(self):
        # add member
        snippets.add_member_to_key_ring_policy(
            self.project_id, self.location, self.keyring_id, self.member, self.role
        )
        policy = snippets.get_key_ring_policy(
            self.project_id, self.location, self.keyring_id
        )
        ...
        assert found
        # remove member
        snippets.remove_member_from_key_ring_policy(
            self.project_id, self.location, self.keyring_id, self.member, self.role
        )
        policy = snippets.get_key_ring_policy(
            self.project_id, self.location, self.keyring_id
    ...
    def test_key_policy(self):
        ...
        policy = snippets.get_crypto_key_policy(
            self.project_id, self.location, self.keyring_id, self.symId
        )
        found = False
        for b in list(policy.bindings):
            if b.role == self.role and self.member in b.members:
    ...
    def test_symmetric_encrypt_decrypt(self):
        cipher_bytes = snippets.encrypt_symmetric(
            self.project_id,
            self.location,
            self.keyring_id,
            self.symId,
        ...
        plain_bytes = snippets.decrypt_symmetric(
            self.project_id, self.location, self.keyring_id, self.symId, cipher_bytes
        )
