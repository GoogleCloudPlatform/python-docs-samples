...
class TestContainerAnalysisSamples:
    ...
    def test_poll_discovery_occurrence(self):
        ...
        grafeas_client.create_note(
            grafeas_client.project_path(PROJECT_ID), note_id, note
        )
        occurrence = {
            "note_name": grafeas_client.note_path(PROJECT_ID, note_id),
            "resource_uri": self.image_url,
            "discovery": {
                "analysis_status": DiscoveryOccurrence.AnalysisStatus.FINISHED_SUCCESS
            },
        }
        created = grafeas_client.create_occurrence(
            grafeas_client.project_path(PROJECT_ID), occurrence
        )
