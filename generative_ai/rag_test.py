...
def test_corpus_fixture():
    ...
    corpus = rag.create_corpus("test_corpus")
    yield corpus
...
def uploaded_file_fixture(test_corpus, test_file):
    ...
    rag_file = rag.upload_file(test_corpus.name, test_file)
...
def test_create_corpus():
    corpus = rag.create_corpus("test_create_corpus")
    assert corpus.display_name == "test_create_corpus"
    rag.delete_corpus(corpus.name)


def test_get_corpus(test_corpus):
    retrieved_corpus = rag.get_corpus(test_corpus.name)
...
def test_import_files(test_corpus):
    response = rag.import_files(test_corpus.name, [GCS_FILE])
    assert response.imported_rag_files_count > 0
    rag.delete_file(response.imported_rag_files[0].name)
...
async def test_import_files_async(test_corpus):
    ...
    rag.delete_file(result.imported_rag_files[0].name)


def test_get_file(uploaded_file):
    retrieved_file = rag.get_file(uploaded_file.name)
    assert retrieved_file.name == uploaded_file.name


def test_list_files(test_corpus, uploaded_file):
    files = rag.list_files(test_corpus.name)
    assert any(f.name == uploaded_file.name for f in files)


def test_retrieval_query(test_corpus):
    response = rag.retrieval_query(test_corpus.name)
    assert response
    assert response.contexts


def test_generate_content_with_rag(test_corpus):
    response = rag.generate_content_with_rag(test_corpus.name)
    assert response
    assert response.text


def test_quickstart():
    corpus, response = rag.quickstart("test_corpus_quickstart", [GCS_FILE])
    assert response
    assert response.text
    rag.delete_corpus(corpus.name)
