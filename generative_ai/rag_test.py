...
def test_corpus():
    ...
    corpus = rag.create_corpus(PROJECT_ID, LOCATION, CORPUS_NAME)
    yield corpus
    rag.delete_corpus(PROJECT_ID, LOCATION, corpus.name)


@pytest.fixture
def uploaded_file(test_corpus):
    """Uploads a file to the corpus and deletes it after the test."""
    rag_file = rag.upload_file(PROJECT_ID, LOCATION, test_corpus.name, test_file)
    yield rag_file
    rag.delete_file(PROJECT_ID, LOCATION, rag_file.name)


def test_create_corpus():
    corpus = rag.create_corpus(PROJECT_ID, LOCATION, "test_create_corpus")
    assert corpus.display_name == "test_create_corpus"
    rag.delete_corpus(PROJECT_ID, LOCATION, corpus.name)


def test_get_corpus(test_corpus):
    retrieved_corpus = rag.get_corpus(PROJECT_ID, LOCATION, test_corpus.name)
    assert retrieved_corpus.name == test_corpus.name


def test_list_corpora():
    corpora = rag.list_corpora(PROJECT_ID, LOCATION)
...
def test_import_files(test_corpus):
    response = rag.import_files(PROJECT_ID, LOCATION, test_corpus.name, test_file)
    assert response.imported_rag_files_count > 0


def test_get_file(uploaded_file):
    retrieved_file = rag.get_file(PROJECT_ID, LOCATION, uploaded_file.name)
    assert retrieved_file.name == uploaded_file.name


def test_list_files(test_corpus, uploaded_file):
    files = rag.list_files(PROJECT_ID, LOCATION, test_corpus.name)
    assert any(f.name == uploaded_file.name for f in files)


def test_retrieval_query(test_corpus):
    response = rag.retrieval_query(PROJECT_ID, LOCATION, test_corpus.name, "test query")
    assert len(response.results) > 0


def test_generate_content_with_rag():
    corpus, response = rag.generate_content_with_rag(
        PROJECT_ID, LOCATION, "test_corpus_generate_content", [test_file]
    )
    assert response
    rag.delete_corpus(PROJECT_ID, LOCATION, corpus.name)
